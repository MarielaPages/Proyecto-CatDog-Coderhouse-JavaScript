...
let productos = [
    {id: 1, nombre: "Neat brown wooden dog bed", precio: 150, imagen: "../media/fotos/wooden-dog-bed.jpg"},
    {id: 2, nombre: "Black dog bed", precio: 90, imagen: "../media/fotos/black-dog-bed.jpg"},
    {id: 3, nombre:"Waterproof dog clothing", precio: 80, imagen:"../media/fotos/Waterproof-Dog-Clothes.jpg_.webp"},
    {id: 4, nombre:"Military wool dog bed", precio: 100, imagen:"../media/fotos/military-wool-dog-beds.jpg"},
    {id: 5, nombre:"Wooden dog house bed", precio: 120, imagen:"../media/fotos/wooden-dog-house-bed.jpg"},
    {id: 6, nombre:"Small brown wooden dog bed", precio: 100, imagen:"../media/fotos/small-brown-wooden-dog-bed.jpg"}
]
...
//Agrego nuevos productos a la pagina 
class NuevosProductos{
    constructor(id, nombreProd, precioProd, imagen){
        this.id = id;
...
nuevosIngresos.push(new NuevosProductos(7, "Bone dog toy", 30, "../media/fotos/bonetoy.jpg"));
nuevosIngresos.push(new NuevosProductos(8, "Dog pyjama", 50, "../media/fotos/dog-pyjamas.jpg"));
nuevosIngresos.push(new NuevosProductos(9, "Wooden dog house", 120, "../media/fotos/woodenDogHouse.jpg"));

let moreDogProducts = document.getElementById("moreDogProducts");
moreDogProducts.addEventListener("click", agregarProdutos);
function agregarProdutos(){
    let contenedor = document.getElementById("contenedorProductos");
    nuevosIngresos.forEach((producto, index) => {
        ...
        card.innerHTML = `<a href="${producto.imagen}" data-lightbox="roadtrip">
                            ...
                            <p class="lightLetters bigProdLetters">${(producto.nombre).toUpperCase()}</p>
                            <p>usd ${producto.precio}</p>
                            <button class="prodButton borderRadius" onclick="abrirCarrito2(${index})">Buy</button>`
        contenedor.appendChild(card)
        moreDogProducts.setAttribute("style", "display:None");
    })
}
...
if (localStorage.getItem("carrito actualizado dogs")) {
    cart = JSON.parse(localStorage.getItem("carrito actualizado dogs"));
    actualizarCarrito();
}

//Carrito con primeros productos
...
const abrirCarrito2 = function (indexProducts){
    const indiceEncontradoCarrito = cart.findIndex((elemento) => {
        return elemento.id === nuevosIngresos[indexProducts].id;
      });
      if (indiceEncontradoCarrito === -1) {
        //agrego el producto
        const productoAgregar = nuevosIngresos[indexProducts];
        productoAgregar.cantidad = 1;
        cart.push(productoAgregar);
        actualizarCarrito();
      } else {
...
function actualizarCarrito(){
    ...
    localStorage.setItem("carrito actualizado dogs", JSON.stringify(cart));
}
